...
if (!empty($_POST)) {   // si le formulaire a été envoyé
	...
	if (empty($contenu)) { // si la variable est vide c'est qu'il n'y a pas d'erreur sur notre formulaire : on peut faire l'insertion en BDD
		...
		$resultat = $pdo->prepare("INSERT INTO experiences (titre, date_debut, date_fin, description) VALUES (:titre, :date_debut, :date_fin, :description)");
		...
		if ($succes) {
			// Si l'experience a été ajouté alert succes
			$contenu .= '<div class="alert alert-success">L\'experience a été ajouté.</div>';
		} else {
			// sinon danger l'experience n'a pas été ajouté
			$contenu .= '<div class="alert alert-danger">L\'experience n\'a pas été ajouté.</div>';
		}
	}
}
...
// 3- Formulaire de d'experience 
?>
    <div class="container">
		<div class="row alert bg-info">
			<h1 class ="mt-4; text-white ">Ajouter un experience</h1>
		</div>
